...
class Expand:
    ...
    def __call__(self, image, boxes, labels):
        ...
        boxes = boxes.copy()
        """
        copy()を使う理由は、Pythonでリストや配列などのミュータブル（変更可能な）
        オブジェクトを扱う際、直接代入を行うと元のオブジェクトも変更されてしまう
        ため。boxesの元のデータを保持しながら、変更を加える必要がある場合には、
        そのコピーを作成して作業を行うことが一般的。これにより、元のデータを別の
        目的で再利用する場合や、デバッグの際に元のデータを参照する必要がある場合に
        便利。
        """
...
class RandomMirror:
    def __call__(self, image, boxes, classes):
        ...
        if random.randint(2):
            ...
            """
            0::2はxminとxmaxの位置を指し、2::-2はxmaxとxminの順
            に取得するため、実際にはxmaxの値がxminに、xminの値が
            xmaxになるように計算してる。
            """
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                # bboxとサンプリングされたパッチのセンターを合わせる
                centers = (boxes[:, :2] + boxes[:, 2:]) / 2.0

                # 左上側にあるすべてのbboxをマスクする
                m1 = (rect[0] < centers[:, 0]) * (rect[1] < centers[:, 1])

                # 右下側にあるすべてのbboxをマスクする
